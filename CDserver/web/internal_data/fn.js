/**
 * This file is part of CDserver Server - Calendar Server
...
function get_collections(user, password, collection, callback) {
    ...
    request.send('<?xml version="1.0" encoding="utf-8" ?>' +
                 '<propfind xmlns="DAV:" xmlns:C="urn:ietf:params:xml:ns:caldav" ' +
                         ...
                         'xmlns:CDserver="http://CDserver.org/ns/">' +
                     '<prop>' +
                         '<resourcetype />' +
                         '<CDserver:displayname />' +
                         '<I:calendar-color />' +
    ...
    this.show = function() {
        ...
        collections_req = get_collections(user, password, collection, function(collections1, error) {
            ...
            if (error) {
                ...
                let collectionsNew = collections1;
                if (collectionsNew.length === 0) {
                    let href = collection.href + random_uuid() + "/";
                    let color = "#" + random_hex(6);
                    let displayname = "contacts";
                    let description = "description";
                    let type = "ADDRESSBOOK";
                    let sane_color = color.trim();
                    if (sane_color) {
                        let color_match = COLOR_RE.exec(sane_color);
                        if (!color_match) {
                    ...
                    let collectiont = new Collection(href, type, displayname, description, sane_color);
                    let callback = function(error1) {
                        return
                    };
                    create_edit_req = create_collection(user, password, collectiont, callback);
                    let create_collection_scene = new CreateEditCollectionScene(user, password, collection);
                    push_scene(create_collection_scene, false);
                    update();
                }
